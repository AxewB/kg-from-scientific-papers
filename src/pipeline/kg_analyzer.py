...
class KGAnalyzer:
    def __init__(self):
        self.resolver: EntityResolver = EntityResolver()
    ...
    def analyze(self, result: NLPResult) -> dict[str, float]:
        ...
        #  metrics
        avg_entity_mentions = total_entity_mentions / total_sent if total_sent else 0
        avg_relations = total_rel / total_sent if total_sent else 0
        relation_coverage = sentences_with_rel / total_sent if total_sent else 0
        unique_entity_ratio = unique_entities / total_entity_mentions if total_entity_mentions else 0
